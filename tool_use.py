...
def extract_reply(text):
    ...
    if match:
        ...
        return text  # Return original text if no reply tags found
...
def simple_chat():
    ...
    print("Welcome to Acme Co Support! Type 'quit' to exit.")
    user_message = input("\nUser: ")
    ...
    while user_message != "quit":
        ...
        if response.stop_reason == "tool_use":
            # Get the tool use from the response
            ...
            # If Claude does NOT want to use a tool, print the text response
            model_reply = extract_reply(response.content[0].text)
            print("\nAcme Co Support: " + model_reply)
